...
def test_add_brand(login, product):
    ...
    login.get_by_placeholder("Enter name").fill("Toyota")
...
def test_add_category(login, product):
    ...
    login.get_by_placeholder("Enter name").fill("Toyota-Camry")
    login.get_by_role("button", name="Submit").click()
    login.wait_for_timeout(10000)
...
def test_create_Product(login, product):
    ...
    # Product name
    login.get_by_placeholder("Enter name").fill("June-CAR")
    ...
    login.get_by_role("option", name="Toyota-Camry", exact=True).click()
    ...
    login.get_by_role("option", name="Toyota", exact=True).click()
